...
import {
  ...
  checkAuth // Make sure checkAuth is imported
} from "@/lib/api";
import { 
  FiUploadCloud, 
  FiTrash2, 
...
export default function ServiceGalleryPage() {
  ...
  // ================= LOAD GALLERY =================
  useEffect(() => {
    if (!selectedService || isAuthLoading) {
      setGallery([]);
      return;
    }

    getServiceGallery(selectedService).then((res) =>
      setGallery(res.data)
    );
  }, [selectedService, isAuthLoading]);
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadServiceGallery(selectedService, data, {
      ...
      // Refresh gallery
      const res = await getServiceGallery(selectedService);
      setGallery(res.data);
  ...
  const handleDelete = async (id) => {
    ...
    await deleteServiceGalleryImage(id);
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(
      selectedImages.map((id) => deleteServiceGalleryImage(id))
    );

    setGallery(gallery.filter((img) => !selectedImages.includes(img._id)));
    setSelectedImages([]);
  };
  ...
  const handleDropReorder = async (e, index) => {
    ...
    try {
        await reorderServiceGallery({ items: payload });
    } catch(err) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-gray-800 pb-20">
      ...
      {/* HEADER */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">Gallery Manager</h1>
        <p className="text-sm text-gray-500 mt-1">Upload and organize images for specific services.</p>
      </div>

      {/* SERVICE SELECTOR */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
        <label className="block text-sm font-semibold text-gray-700 mb-2">Select a Service to Manage</label>
        <div className="relative max-w-md">
          <select
            className="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#1D4F41]/20 focus:border-[#1D4F41] transition-all cursor-pointer font-medium"
            onChange={(e) => {
          ...
          {/* <FiChevronDown className="absolute right-4 top-3.5 text-gray-500 pointer-events-none w-5 h-5" /> */}
        </div>
      </div>
      ...
      {selectedService ? (
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <FiUploadCloud className="text-[#1D4F41]" /> Upload New Images
            </h2>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                 <FiGrid className="text-[#1D4F41]" /> Current Gallery
                 <span className="text-sm font-normal text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full ml-2">{gallery.length} Images</span>
              </h2>
            ...
            {gallery.length === 0 ? (
              <div className="text-center py-12 px-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                  ...
                  <h3 className="text-gray-900 font-medium">No images found</h3>
                  <p className="text-gray-500 text-sm mt-1">Upload some images above to start building the gallery.</p>
              </div>
            ) : (
              <>
        ...
        <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
           ...
           <h2 className="text-lg font-medium text-gray-600">No Service Selected</h2>
           <p className="text-sm text-gray-500 mt-1">Please select a service from the dropdown above to manage its gallery.</p>
        </div>
      )}
    </div>
  );
