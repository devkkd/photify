...
export default function PortfolioPage() {
  ...
  // ================= LOAD =================
  useEffect(() => {
    // Only fetch portfolio if auth is complete
    if (!isAuthLoading) {
      fetchPortfolio();
    }
  }, [isAuthLoading]);
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(selectedImages.map((id) => deletePortfolio(id)));
  ...
  const handleDropReorder = async (e, index) => {
    ...
    try {
      await reorderPortfolio({ items: payload });
    } catch (err) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-gray-800 pb-20">
      ...
      {/* HEADER */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">Portfolio Manager</h1>
        <p className="text-sm text-gray-500 mt-1">Manage and organize your main showcase portfolio.</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <FiUploadCloud className="text-[#1D4F41]" /> Upload New Work
          </h2>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
            <h2 className="text-lg font-semibold flex items-center gap-2">
               <FiGrid className="text-[#1D4F41]" /> Current Portfolio
          ...
          {gallery.length === 0 ? (
            <div className="text-center py-12 px-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                ...
                <h3 className="text-gray-900 font-medium">Your portfolio is empty</h3>
                <p className="text-gray-500 text-sm mt-1">Upload some images above to showcase your best work.</p>
            </div>
          ) : (
            <>
